import styles from "../style/About.module.css"


export default function About() {
    return <>
        <div className={styles.description}> 
            <p>I’m passionate about front-end web development and I've graduated from the IT School course, 
                a 5 months of intense training, where I studied: HTML, CSS, JavaScript, React, Bootstrap.
            </p>
            <p>I am perseverent, hard-working and love to see the results of my work. 
            </p>
            <p>I am familiar with the Agile methodology and would be happy to be part of a team where I can learn
                and develop my knowledge, while working on beautiful interactive websites.
            </p>
        </div>
    </>
}